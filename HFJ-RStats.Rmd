...
First, we need to load the sentencing data into R. The `read_csv()` function loads the data into R, and the assignment operator `<-` saves the data under the name `sentence`, which we will use to refer to it from here forward. Data loaded into R is called a `dataframe`, and we will use that terminology going forward.
...
Very frequently, the data you are more interested or curious in will not be made available 

A very common task in data manip
Let's load in a different dataset from the SAO:

```{r message=FALSE, warning=FALSE}
initiation <- read_csv("Dispositions.csv")
```
